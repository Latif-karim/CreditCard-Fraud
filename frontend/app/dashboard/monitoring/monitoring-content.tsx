...
import {
  ...
  transactionMatchesMonitoringFilters,
...
export function MonitoringContent() {
  ...
  const [highlightTx, setHighlightTx] = useState<string | null>(null);
  ...
  }, [hydrated, searchParams, urlApplied]);
    ...
    useCallback(
      (event: LiveStreamEvent) => {
        if (event.type === "transaction.created") {
          ...
          if (!transactionMatchesMonitoringFilters(tx, { q, status, riskMin, country })) return;
          setData((prev) => {
            if (!prev) return prev;
            if (prev.items.some((row) => row.id === tx.id)) return prev;
            return {
              ...prev,
              items: [tx, ...prev.items].slice(0, prev.per_page),
              total: prev.total + 1,
            };
          });
          return;
        ...
        if (event.type === "transaction.updated") {
          setData((prev) => {
            if (!prev) return prev;
            const idx = prev.items.findIndex((row) => row.id === event.transaction.id);
            if (idx < 0) return prev;
            const items = [...prev.items];
            items[idx] = { ...items[idx], ...event.transaction };
            return { ...prev, items };
          });
        }
      },
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-xs">
                            ...
                            <div className="text-soft max-w-[140px] truncate">{tx.device_id || "—"}</div>
                          </td>
